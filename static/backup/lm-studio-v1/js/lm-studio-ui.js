...
(function () {
  ...
  const serverSettingsBtn = document.querySelector('[data-action="open-server-settings"]');
  ...
  if (serverSettingsBtn) serverSettingsBtn.addEventListener('click', () => openModal(serverSettingsModal));
  ...
  document.querySelectorAll('.lm-settings-nav-item').forEach((item) => {
    item.addEventListener('click', () => {
      const id = item.dataset.settingsPanel;
      document.querySelectorAll('.lm-settings-nav-item').forEach((n) => n.classList.toggle('active', n === item));
      document.querySelectorAll('.lm-settings-panel').forEach((p) => p.classList.toggle('active', p.dataset.settingsPanel === id));
    });
  });
